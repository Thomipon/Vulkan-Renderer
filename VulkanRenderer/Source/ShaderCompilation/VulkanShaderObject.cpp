...
void VulkanShaderObject::initializeGlobalDescriptorSet(const ShaderOffset &offset)
{
	if (buffer)
	{
		const uint32_t bindingIndex = offset.bindingIndex; //typeLayout->getBindingRangeIndexOffset(offset.bindingIndex);

		vk::DescriptorBufferInfo bufferInfo{buffer->vkBuffer, 0, layout->typeLayout->getSize() };

		std::vector<vk::WriteDescriptorSet> descriptorWrites{};
		descriptorWrites.reserve(descriptorSets.size());
		for (const auto& descriptorSet : descriptorSets)
		{
			descriptorWrites.emplace_back(descriptorSet, bindingIndex, offset.bindingArrayElement, 1, VulkanShaderObjectLayout::mapDescriptorType(typeLayout->getBindingRangeType(bindingIndex)), nullptr, &bufferInfo);
		}
		app.device.updateDescriptorSets(descriptorWrites, {});
	}
}
...
{
	initializeGlobalDescriptorSet(ShaderCursor{this}.field("globalParams").getOffset());
}
